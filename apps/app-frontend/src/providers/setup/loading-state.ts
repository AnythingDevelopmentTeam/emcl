import type { LoadingStateProvider } from '@emcl/ui'
import { createLoadingStateCore, provideLoadingState } from '@emcl/ui'

/**
 * Source of truth for the desktop app's loading state.
 *
 * Owns the token-based ref-counter directly (no Pinia store). Consumers
 * obtain the same reactive state via `injectLoadingState()` from `@emcl/ui`.
 *
